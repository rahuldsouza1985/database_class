    ...
    class database {
		...
		private function run_query($query, $parameters, $additional_parameters = false) {
			...
			foreach($parameters as $placeholder => $value) {
				$parameters[$placeholder] = htmlentities($value);
		...
		public function create($parameters, $table) {
			$columns = array();
			foreach($parameters as $placeholder => $value) {
				$columns[] = str_replace(':', '', $placeholder);
			}
			
			return $this->run_query("INSERT INTO $table(".implode(',', $columns).") VALUES(".array_keys($parameters).")", $parameters, 'insert');
		}
		...
		public function single_inflate(&$dataset, $table_name, $id, $columns) {
			...
			$row = $this->run_query("SELECT ".implode(",", $columns)." FROM $table_name WHERE $table_name.id = :id", array(':id' => $id));
